...
class EccoNMF(ToolkitPlugin):
    ...
    def spec(self) -> List[FieldSpec]:
        return [
            FieldSpec(
                ...
                label="Model (Ecco / HF)",
                type="select",
                options=self.DEFAULT_MODELS + ["custom"],
                help="Choose a default model, or pick custom to type any HF model id.",
                default=self.DEFAULT_MODELS[0],
            ),
            FieldSpec(
                key="model_name",
                label="Custom HF model name (used only if Model=custom)",
                type="text",
                required=False,
                help="Example: EleutherAI/gpt-neo-125M (must be available in your environment).",
                default="",
            ),
            FieldSpec(
                key="text",
    ...
    @torch.no_grad()
    def run(self, inputs: Dict[str, Any]) -> Dict[str, Any]:
        model_choice = (inputs.get("model_choice") or "").strip() or self.DEFAULT_MODELS[0]
        custom_name = (inputs.get("model_name") or "").strip()

        model_name = custom_name if model_choice == "custom" and custom_name else model_choice
